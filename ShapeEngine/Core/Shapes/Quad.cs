...
public readonly struct Quad : IEquatable<Quad>
{
    ...
    {
        var ab = Segment.GetClosestPointSegmentPoint(a, b, p);
        var bc = Segment.GetClosestPointSegmentPoint(b, c, p);
        var cd = Segment.GetClosestPointSegmentPoint(c, d, p);
        var da = Segment.GetClosestPointSegmentPoint(d, a, p);
    ...
    public ClosestDistance GetClosestDistanceTo(Triangle triangle)
    {
        ...
        next = Triangle.GetClosestPointTrianglePoint(triangle.A, triangle.B, triangle.C, A);
        disSq = (next - A).LengthSquared();
        if (disSq < minDisSq)
        {
            minDisSq = disSq;
        ...
        next = Triangle.GetClosestPointTrianglePoint(triangle.A, triangle.B, triangle.C, B);
        disSq = (next - B).LengthSquared();
        if (disSq < minDisSq)
        {
            minDisSq = disSq;
        ...
        next = Triangle.GetClosestPointTrianglePoint(triangle.A, triangle.B, triangle.C, C);
        disSq = (next - C).LengthSquared();
        if (disSq < minDisSq)
        {
            minDisSq = disSq;
        ...
        next = Triangle.GetClosestPointTrianglePoint(triangle.A, triangle.B, triangle.C, D);
        disSq = (next - D).LengthSquared();
        if (disSq < minDisSq)
        {
            cpSelf = D;
    ...
    public ClosestSegment GetClosestSegment(Vector2 p)
    {
        ...
        var c = Segment.GetClosestPointSegmentPoint(A, B, p); // SegmentAToB.GetClosestPoint(p);
        ...
        c =  Segment.GetClosestPointSegmentPoint(B, C, p);
        ...
        c =  Segment.GetClosestPointSegmentPoint(C, D, p);
        disSq = (c - p).LengthSquared();
        if (disSq < minD)
        {
        ...
        c =  Segment.GetClosestPointSegmentPoint(D, A, p);
        disSq = (c - p).LengthSquared();
        if (disSq < minD)
        {
