...
using Ray = ShapeEngine.Geometry.Ray.Ray;
...
public readonly struct Rect : IEquatable<Rect>
{
    ...
    public readonly Segment.Segment LeftSegment => new(TopLeft, BottomLeft);
    public readonly Segment.Segment BottomSegment => new(BottomLeft, BottomRight);
    public readonly Segment.Segment RightSegment => new(BottomRight, TopRight);
    public readonly Segment.Segment TopSegment => new(TopRight, TopLeft);
    ...
    public bool SeperateAxis(Vector2 axisStart, Vector2 axisEnd)
    {
        ...
        var edgeARange = Segment.Segment.ProjectSegment(edgeAStart, edgeAEnd, n);
        var edgeBRange = Segment.Segment.ProjectSegment(edgeBStart, edgeBEnd, n);
        var rProjection = RangeHull(edgeARange, edgeBRange);

        var axisRange = Segment.Segment.ProjectSegment(axisStart, axisEnd, n);
    ...
    /// <returns></returns>
    public Polygon.Polygon Rotate(float angleDeg, AnchorPoint alignement)
    {
        var poly = ToPolygon();
    ...
    public Segments GetEdges() 
    {
        ...
        Segment.Segment left = new(A, B);
        Segment.Segment bottom = new(B, C);
        Segment.Segment right = new(C, D);
        Segment.Segment top = new(D, A);
        return new() { left, bottom, right, top };
    }

    public Triangulation Triangulate()
    {
        Triangle.Triangle a = new(TopLeft, BottomLeft, BottomRight);
        Triangle.Triangle b = new(TopLeft, BottomRight, TopRight);
        return new Triangulation() { a, b };
    }

    public Polygon.Polygon GetSlantedCornerPoints(float tlCorner, float trCorner, float brCorner, float blCorner)
    {
        var tl = TopLeft;
        var tr = TopRight;
        var br = BottomRight;
    ...
    public Polygon.Polygon GetSlantedCornerPointsRelative(float tlCorner, float trCorner, float brCorner, float blCorner)
    {
        var tl = TopLeft;
        var tr = TopRight;
        var br = BottomRight;
    ...
    /// </summary>
    /// <returns></returns>
    public readonly Polygon.Polygon GetPointsRelative(Vector2 pos)
    {
    ...
    public static Rect FromCircle(Circle.Circle c) => new(c.Center, new Size(c.Radius, c.Radius), new (0.5f, 0.5f));
    ...
    public static bool ContainsRectCircle(Vector2 topLeft, Vector2 bottomRight, Vector2 circleCenter, float circleRadius)
    {
        ...
        var result = Segment.Segment.IntersectSegmentCircle(a, b, circleCenter, circleRadius);
        if (result.a.Valid || result.b.Valid) return false;
            
        result = Segment.Segment.IntersectSegmentCircle(b, c, circleCenter, circleRadius);
        if (result.a.Valid || result.b.Valid) return false;
        
        result = Segment.Segment.IntersectSegmentCircle(c, d, circleCenter, circleRadius);
        if (result.a.Valid || result.b.Valid) return false;

        result = Segment.Segment.IntersectSegmentCircle(d, a, circleCenter, circleRadius);
        if (result.a.Valid || result.b.Valid) return false;
        return true;
    ...
    public readonly (bool outOfBounds, Vector2 newPos) BoundsWrapAround(Circle.Circle boundingCircle)
    {
    ...
    public readonly BoundsCollisionInfo BoundsCollision(Circle.Circle boundingCircle)
    {
        var pos = boundingCircle.Center;
        var radius = boundingCircle.Radius;
    ...
    public readonly bool OverlapShape(Segment.Segment s) => s.OverlapShape(this);
    public readonly bool OverlapShape(Circle.Circle c) => c.OverlapShape(this);
    public readonly bool OverlapShape(Triangle.Triangle t) => t.OverlapShape(this);
    public readonly bool OverlapShape(Quad.Quad q) => q.OverlapShape(this);
    ...
    public readonly bool OverlapShape(Polygon.Polygon poly)
    {
    ...
    public readonly bool OverlapShape(Polyline.Polyline pl)
    {
