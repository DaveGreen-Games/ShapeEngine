...
public readonly partial struct Quad
{
    ...
    /// </summary>
    /// <param name="result">The `Segments` collection to populate with stripe segments clipped to this quad.</param>
    ...
    /// <param name="result">The `Segments` collection to populate with stripe segments that lie inside this quad but outside the insideShape.</param>
    ...
    public bool GenerateStripedSegments<T>(Segments result, T insideShape, float spacing, float angleDeg, float spacingOffset = 0f)  where T : IClosedShapeTypeProvider
    {
        ...
        if (insideShape is Triangle triangle)
        {
            for (int i = 0; i < steps; i++)
            {
                var outsideShapePoints = Ray.IntersectRayQuad(cur, rayDir, A, B, C, D);
        ...
        else if (insideShape is Polygon polygon)
        {
            for (int i = 0; i < steps; i++)
            {
                var outsideShapePoints = Line.IntersectLineQuad(cur, rayDir, A, B, C, D);
                if (!outsideShapePoints.a.Valid || !outsideShapePoints.b.Valid)
                {
                    cur += dir * spacing;
                    continue;
    ...
    /// <summary>
    /// Draws a striped pattern inside this quad while excluding the area covered by the specified inner shape.
    /// </summary>
    /// <typeparam name="T">
    ...
    public void DrawStriped(Polygon insideShape, float spacing, float angleDeg, LineDrawingInfo striped, float spacingOffset = 0f)
    {
        ...
        for (int i = 0; i < steps; i++)
        {
            var outsideShapePoints = Line.IntersectLineQuad(cur, lineDir, A, B, C, D);
            if (!outsideShapePoints.a.Valid || !outsideShapePoints.b.Valid)
            {
                cur += dir * spacing;
                continue;
