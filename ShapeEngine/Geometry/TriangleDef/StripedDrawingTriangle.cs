...
public readonly partial struct Triangle
{
    ...
    /// </summary>
    /// <param name="result">The `Segments` collection to populate with stripe segments clipped to this triangle.</param>
    ...
    /// <param name="result">The `Segments` collection to populate with stripe segments that lie inside this triangle but outside the insideShape.</param>
    ...
    public bool GenerateStripedSegments<T>(Segments result, T insideShape, float spacing, float angleDeg, float spacingOffset = 0f)  where T : IClosedShapeTypeProvider
    {
        ...
        if (insideShape is Triangle triangle)
        {
            for (int i = 0; i < steps; i++)
            {
                var outsideShapePoints = Ray.IntersectRayTriangle(cur, rayDir, A, B, C);
        ...
        else if (insideShape is Polygon polygon)
        {
            for (int i = 0; i < steps; i++)
            {
                var outsideShapePoints = Line.IntersectLineTriangle(cur, rayDir, A, B, C);
                if (!outsideShapePoints.a.Valid || !outsideShapePoints.b.Valid)
                {
                    cur += dir * spacing;
                    continue;
    ...
    /// <summary>
    /// Draws a striped pattern inside this triangle while excluding the area covered by the specified inner shape.
    /// </summary>
    /// <typeparam name="T">
    ...
    public void DrawStriped(Polygon insideShape, float spacing, float angleDeg, LineDrawingInfo striped, float spacingOffset = 0f)
    {
        if (spacing <= 0) return;
        var center = GetCentroid();
        GetFurthestVertex(center, out float disSq, out int _);
        ...
        for (int i = 0; i < steps; i++)
        {
            var outsideShapePoints = Line.IntersectLineTriangle(cur, lineDir, A, B, C);
            if (!outsideShapePoints.a.Valid || !outsideShapePoints.b.Valid)
            {
                cur += dir * spacing;
                continue;
