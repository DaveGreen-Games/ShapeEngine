...
public readonly struct Line
{
    ...
    public Segment.Segment ToSegment(float length)
    {
        ...
        return new Segment.Segment(Point - Direction * length * 0.5f, Point + Direction * length * 0.5f, Normal);
    }
    ...
    /// </remarks>
    public Ray.Ray ToRay(bool reversed = false) => reversed ? new Ray.Ray(Point, -Direction, -Normal) : new Ray.Ray(Point, Direction, Normal);
    ...
    /// </remarks>
    public Rect.Rect GetBoundingBox() { return new(Point - Direction * MaxLength * 0.5f, Point + Direction * MaxLength * 0.5f); }
    /// <summary>
    ...
    /// </remarks>
    public Rect.Rect GetBoundingBox(float length) { return new(Point - Direction * length * 0.5f, Point + Direction * length * 0.5f); }
    ...
    {
        var result = Segment.Segment.GetClosestPointSegmentLine(other.Start, other.End, Point, Direction, out var disSquared);
        return new(
    ...
    public static (CollisionPoint p, float t) IntersectLineSegmentInfo(Vector2 linePoint, Vector2 lineDirection, Vector2 segmentStart, Vector2 segmentEnd)
    {
        ...
        if (Segment.Segment.IsPointOnSegment(intersection, segmentStart, segmentEnd))
        {
    ...
    public static CollisionPoint IntersectLineSegment(Vector2 linePoint, Vector2 lineDirection, Vector2 segmentStart, Vector2 segmentEnd)
    {
        ...
        var result = Ray.Ray.IntersectRaySegment(linePoint, lineDirection, segmentStart, segmentEnd);
        if (result.Valid) return result;
        return Ray.Ray.IntersectRaySegment(linePoint, -lineDirection, segmentEnd, segmentStart);
    ...
    /// </remarks>
    public static CollisionPoints? IntersectLineSegments(Vector2 linePoint, Vector2 lineDirection, List<Segment.Segment> segments, int maxCollisionPoints = -1)
    {
    ...
    /// This is a convenience method that uses the line's point and direction.
    /// </remarks>
    public CollisionPoint IntersectSegment(Segment.Segment segment) => IntersectLineSegment(Point, Direction, segment.Start, segment.End, segment.Normal);
    /// <summary>
    ...
    /// A <see cref="CollisionPoint"/> at the intersection, or an invalid <see cref="CollisionPoint"/> if the intersection does not lie in the direction of the ray.
    /// </returns>
    public CollisionPoint IntersectRay(Ray.Ray otherRay) => IntersectLineRay(Point, Direction, otherRay.Point, otherRay.Direction, otherRay.Normal);
    /// <summary>
    ...
    /// If no intersection exists, both points are invalid.
    /// </returns>
    public (CollisionPoint a, CollisionPoint b) IntersectCircle(Circle.Circle otherCircle) => IntersectLineCircle(Point, Direction, otherCircle.Center, otherCircle.Radius);
    /// <summary>
    /// Computes the intersection points between this infinite line and a circle.
    /// </summary>
    /// <param name="circleCenter">The center of the circle.</param>
    ...
    /// If no intersection exists, both points are invalid.
    /// </returns>
    public (CollisionPoint a, CollisionPoint b) IntersectTriangle(Triangle.Triangle triangle) => IntersectLineTriangle(Point, Direction, triangle.A, triangle.B, triangle.C);
    /// <summary>
    ...
    /// If no intersection exists, both points are invalid.
    /// </returns>
    public (CollisionPoint a, CollisionPoint b) IntersectQuad(Quad.Quad quad) => IntersectLineQuad(Point, Direction, quad.A, quad.B, quad.C, quad.D);
    /// <summary>
    ...
    /// If no intersection exists, both points are invalid.
    /// </returns>
    public (CollisionPoint a, CollisionPoint b) IntersectRect(Rect.Rect rect) => IntersectLineQuad(Point, Direction, rect.A, rect.B, rect.C, rect.D);
    /// <summary>
    ...
    /// A <see cref="CollisionPoints"/> object containing the intersection points, or null if no intersections exist.
    /// </returns>
    public CollisionPoints? IntersectPolygon(Polygon.Polygon polygon, int maxCollisionPoints = -1) => IntersectLinePolygon(Point, Direction, polygon, maxCollisionPoints);
    /// <summary>
    ...
    /// A <see cref="CollisionPoints"/> object containing the intersection points, or null if no intersections exist.
    /// </returns>
    public CollisionPoints? IntersectPolyline(Polyline.Polyline polyline, int maxCollisionPoints = -1) => IntersectLinePolyline(Point, Direction, polyline, maxCollisionPoints);
    ...
    /// This is a convenience method that uses the line's point and direction.
    /// </remarks>
    public CollisionPoints? IntersectSegments(List<Segment.Segment> segments, int maxCollisionPoints = -1) => IntersectLineSegments(Point, Direction, segments, maxCollisionPoints);
    ...
    /// This is a convenience method that uses the line's point and direction.
    /// </remarks>
    public CollisionPoints? IntersectShape(Segment.Segment segment)
    {
    ...
    /// This is a convenience method that uses the line's point and direction.
    /// </remarks>
    public CollisionPoints? IntersectShape(Ray.Ray ray)
    {
    ...
    /// </remarks>
    public CollisionPoints? IntersectShape(Polygon.Polygon p, int maxCollisionPoints = -1) => IntersectLinePolygon(Point, Direction, p, maxCollisionPoints);
    /// <summary>
    ...
    /// </remarks>
    public CollisionPoints? IntersectShape(Polyline.Polyline pl, int maxCollisionPoints = -1) => IntersectLinePolyline(Point, Direction, pl, maxCollisionPoints);
    /// <summary>
    ...
    /// </remarks>
    public int IntersectShape(Segment.Segment s, ref CollisionPoints points)
    {
