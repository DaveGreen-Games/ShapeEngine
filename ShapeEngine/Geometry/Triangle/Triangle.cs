...
public readonly struct Triangle : IEquatable<Triangle>
{
    ...
    public readonly Segment.Segment SegmentAToB => new(A, B);
    public readonly Segment.Segment SegmentBToC => new(B, C);
    public readonly Segment.Segment SegmentCToA => new(C, A);
    ...
    public Triangle(Vector2 p, Segment.Segment s)
    {
    ...
    public Rect.Rect GetBoundingBox() { return new Rect.Rect(A.X, A.Y, 0, 0).Enlarge(B).Enlarge(C); }
    public Circle.Circle GetCircumCircle()
    {
        //alternative variant
        //coding train
    ...
    public Triangulation Triangulate(int pointCount)
    {
        ...
        return Polygon.Polygon.TriangulateDelaunay(points);
    }
    ...
    public static bool ContainsTriangleCircle(Vector2 tA, Vector2 tB, Vector2 tC, Vector2 circleCenter, float circleRadius)
    {
        ...
        var result = Segment.Segment.IntersectSegmentCircle(tA, tB, circleCenter, circleRadius);
        if (result.a.Valid || result.b.Valid) return false;
            
        result = Segment.Segment.IntersectSegmentCircle(tB, tC, circleCenter, circleRadius);
        if (result.a.Valid || result.b.Valid) return false;
        
        result = Segment.Segment.IntersectSegmentCircle(tC, tA, circleCenter, circleRadius);
        if (result.a.Valid || result.b.Valid) return false;
    ...
    {
        return Segment.Segment.OverlapSegmentTriangle(segmentStart, segmentEnd, a, b, c);
    }
    ...
    {
        return Ray.Ray.OverlapRayTriangle(rayPoint, rayDirection, a, b, c);
    }
    public static bool OverlapTriangleCircle(Vector2 a, Vector2 b, Vector2 c, Vector2 circleCenter, float circleRadius)
    {
        return Circle.Circle.OverlapCircleTriangle(circleCenter, circleRadius, a, b, c);
    }
    public static bool OverlapTriangleTriangle(Vector2 a1, Vector2 b1, Vector2 c1,  Vector2 a2, Vector2 b2, Vector2 c2)
    {
        ...
        if( Segment.Segment.OverlapSegmentSegment(a1, b1, a2, b2) ) return true;
        if( Segment.Segment.OverlapSegmentSegment(a1, b1, b2, c2) ) return true;
        if( Segment.Segment.OverlapSegmentSegment(a1, b1, c2, a2) ) return true;
        
        if( Segment.Segment.OverlapSegmentSegment(b1, c1, a2, b2) ) return true;
        if( Segment.Segment.OverlapSegmentSegment(b1, c1, b2, c2) ) return true;
        if( Segment.Segment.OverlapSegmentSegment(b1, c1, c2, a2) ) return true;
        
        if( Segment.Segment.OverlapSegmentSegment(c1, a1, a2, b2) ) return true;
        if( Segment.Segment.OverlapSegmentSegment(c1, a1, b2, c2) ) return true;
        if( Segment.Segment.OverlapSegmentSegment(c1, a1, c2, a2) ) return true;
    ...
    public static bool OverlapTriangleQuad(Vector2 a, Vector2 b, Vector2 c, Vector2 qa, Vector2 qb, Vector2 qc, Vector2 qd)
    {
        ...
        if (Quad.Quad.ContainsQuadPoint(qa, qb, qc,qd,a)) return true;
        ...
        if(Segment.Segment.OverlapSegmentSegment(c, a, qa, qb) ) return true;
        if(Segment.Segment.OverlapSegmentSegment(c, a, qb, qc) ) return true;
        if(Segment.Segment.OverlapSegmentSegment(c, a, qc, qd) ) return true;
        if(Segment.Segment.OverlapSegmentSegment(c, a, qd, qa) ) return true;
    ...
    public static bool OverlapTrianglePolygon(Vector2 a, Vector2 b, Vector2 c, List<Vector2> points)
    {
        ...
        for (var i = 0; i < points.Count; i++)
        {
            ...
            if( Segment.Segment.OverlapSegmentSegment(c, a, p1, p2) ) return true;
    ...
    public static bool OverlapTriangleSegments(Vector2 a, Vector2 b, Vector2 c, List<Segment.Segment> segments)
    {
        ...
        for (var i = 0; i < segments.Count; i++)
        {
            ...
            if( Segment.Segment.OverlapSegmentSegment(c, a, segment.Start, segment.End) ) return true;
        }
        return false;
    }
    ...
    public CollisionPoints? IntersectShape(Segments segments)
    {
        ...
        foreach (var seg in segments)
        {
            ...
            result = Segment.Segment.IntersectSegmentSegment(C, A, seg.Start, seg.End);
            if (result.Valid)
            {
                points ??= new();
                points.Add((CollisionPoint)result);
    ...
    public CollisionPoints? IntersectShape(Ray.Ray r)
    {
        ...
        result = Segment.Segment.IntersectSegmentRay(C, A, r.Point, r.Direction, r.Normal);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
    ...
    public CollisionPoints? IntersectShape(Line.Line l)
    {
        ...
        result = Segment.Segment.IntersectSegmentLine(C, A, l.Point, l.Direction, l.Normal);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
    ...
    public CollisionPoints? IntersectShape(Segment.Segment s)
    {
        ...
        result = Segment.Segment.IntersectSegmentSegment(C, A, s.Start, s.End);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
    ...
    public CollisionPoints? IntersectShape(Circle.Circle c)
    {
        ...
        result = Segment.Segment.IntersectSegmentCircle(C, A, c.Center, c.Radius);
        if (result.a.Valid || result.b.Valid)
        {
            points ??= new();
            if(result.a.Valid) points.Add(result.a);
    ...
    public CollisionPoints? IntersectShape(Triangle t)
    {
        ...
        result = Segment.Segment.IntersectSegmentSegment(C, A, t.A, t.B);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, t.B, t.C);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, t.C, t.A);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
    ...
    public CollisionPoints? IntersectShape(Rect.Rect r)
    {
        ...
        result = Segment.Segment.IntersectSegmentSegment(C, A, a, b);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, b, c);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, c, d);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, d, a);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
    ...
    public CollisionPoints? IntersectShape(Quad.Quad q)
    {
        ...
        result = Segment.Segment.IntersectSegmentSegment(C, A, q.A, q.B);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, q.B, q.C);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, q.C, q.D);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
        }
        result = Segment.Segment.IntersectSegmentSegment(C, A, q.D, q.A);
        if (result.Valid)
        {
            points ??= new();
            points.Add(result);
    ...
    public CollisionPoints? IntersectShape(Polygon.Polygon p)
    {
        ...
        {
            var colPoint = Segment.Segment.IntersectSegmentSegment(A, B, p[i], p[(i + 1) % p.Count]);
            if (colPoint.Valid)
            {
                points ??= new();
                points.Add(colPoint);
            ...
            colPoint = Segment.Segment.IntersectSegmentSegment(B, C, p[i], p[(i + 1) % p.Count]);
            if (colPoint.Valid)
            {
                points ??= new();
                points.Add(colPoint);
            ...
            colPoint = Segment.Segment.IntersectSegmentSegment(C, A, p[i], p[(i + 1) % p.Count]);
            if (colPoint.Valid)
            {
                points ??= new();
                points.Add(colPoint);
    ...
    public CollisionPoints? IntersectShape(Polyline.Polyline pl)
    {
        ...
        for (var i = 0; i < pl.Count - 1; i++)
        {
            CollisionPoint colPoint = Segment.Segment.IntersectSegmentSegment(A, B, pl[i], pl[(i + 1) % pl.Count]);
            if (colPoint.Valid)
            {
                points ??= new();
                points.Add(colPoint);
            ...
            colPoint = Segment.Segment.IntersectSegmentSegment(B, C, pl[i], pl[(i + 1) % pl.Count]);
            if (colPoint.Valid)
            {
                points ??= new();
                points.Add(colPoint);
            ...
            colPoint = Segment.Segment.IntersectSegmentSegment(C, A, pl[i], pl[(i + 1) % pl.Count]);
            if (colPoint.Valid)
            {
                points ??= new();
                points.Add(colPoint);
    ...
    public int IntersectShape(Polygon.Polygon p, ref CollisionPoints points, bool returnAfterFirstValid = false)
    {
        ...
        for (var i = 0; i < p.Count; i++)
        {
            ...
            result = Segment.Segment.IntersectSegmentSegment(C, A, p[i], p[(i + 1) % p.Count]);
            if (result.Valid)
            {
                points.Add(result);
                if (returnAfterFirstValid) return 1;
    ...
    public int IntersectShape(Polyline.Polyline pl, ref CollisionPoints points, bool returnAfterFirstValid = false)
    {
        ...
        for (var i = 0; i < pl.Count - 1; i++)
        {
            ...
            result = Segment.Segment.IntersectSegmentSegment(C, A, pl[i], pl[(i + 1) % pl.Count]);
            if (result.Valid)
            {
                points.Add(result);
                if (returnAfterFirstValid) return 1;
