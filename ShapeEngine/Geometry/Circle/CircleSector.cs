...
public class CircleSector
{
    ...
    /// </remarks>
    public bool UpdatePolygon(Polygon.Polygon polygon)
    {
    ...
    public Polygon.Polygon? GeneratePolygon() => GeneratePolygon(Center, Radius, RotationRad, AngleSectorRad, ArcPoints);
    /// <summary>
    ...
    public Points.Points? GeneratePoints() => GeneratePoints(Center, Radius, RotationRad, AngleSectorRad, ArcPoints);
    /// <summary>
    ...
    /// <returns>A <see cref="Polygon"/> representing the sector, or null if parameters are invalid.</returns>
    public static Polygon.Polygon? GeneratePolygon(Vector2 center, float radius, float rotationRad, float angleSectorRad, int accuracy = 3)
    {
        if (angleSectorRad <= 0 || radius <= 0) return null;
        var result = new Polygon.Polygon(3 + accuracy);
    ...
    /// <param name="accuracy">The number of points used to approximate the arc. Default is 3.</param>
    /// <returns>A <see cref="Points"/> collection representing the sector, or null if parameters are invalid.</returns>
    public static Points.Points? GeneratePoints(Vector2 center, float radius, float rotationRad, float angleSectorRad, int accuracy = 3)
    {
        if (angleSectorRad <= 0 || radius <= 0) return null;
        var result = new Points.Points(3 + accuracy);
    ...
    public static Segments.Segments? GenerateSegments(Vector2 center, float radius, float rotationRad, float angleSectorRad, int accuracy = 3)
    {
        ...
        {
            var p = center + v;
            result.Add(new Segment.Segment(prevPoint, p));
            prevPoint = p;
            v = v.Rotate(angleStep);
        }
