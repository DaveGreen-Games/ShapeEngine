...
public readonly partial struct Rect
{
    ...
    /// </summary>
    /// <param name="result">The `Segments` collection to populate with stripe segments clipped to this rect.</param>
    ...
    /// <param name="result">The `Segments` collection to populate with stripe segments that lie inside this rect but outside the insideShape.</param>
    ...
    public bool GenerateStripedSegments<T>(Segments result, T insideShape, float spacing, float angleDeg, float spacingOffset = 0f)  where T : IClosedShapeTypeProvider
    {
        ...
        if (insideShape is Triangle triangle)
        {
            for (int i = 0; i < steps; i++)
            {
                var outsideShapePoints = Ray.IntersectRayRect(cur, rayDir, A, B, C, D);
        ...
        else if (insideShape is Polygon polygon)
        {
            for (int i = 0; i < steps; i++)
            {
                var outsideShapePoints = Line.IntersectLineRect(cur, rayDir, A, B, C, D);
                if (!outsideShapePoints.a.Valid || !outsideShapePoints.b.Valid)
                {
                    cur += dir * spacing;
                    continue;
    ...
    {
        if (spacing <= 0) return;
        float maxDimension = (TopLeft - BottomRight).Length();
    ...
    /// <summary>
    /// Draws a striped pattern inside this rect while excluding the area covered by the specified inner shape.
    /// </summary>
    /// <typeparam name="T">
    ...
    public void DrawStriped(Polygon insideShape, float spacing, float angleDeg, LineDrawingInfo striped, float spacingOffset = 0f)
    {
        ...
        for (int i = 0; i < steps; i++)
        {
            var outsideShapePoints = Line.IntersectLineRect(cur, lineDir, A, B, C, D);
            if (!outsideShapePoints.a.Valid || !outsideShapePoints.b.Valid)
            {
                cur += dir * spacing;
                continue;
