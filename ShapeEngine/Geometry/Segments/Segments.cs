...
public partial class Segments : ShapeList<Segment.Segment>
{
    #region Constructors
    public Segments() { }
    public Segments(int capacity) : base(capacity) { }
    //public Segments(IShape shape) { AddRange(shape.GetEdges()); }
    public Segments(IEnumerable<Segment.Segment> edges) { AddRange(edges); }
    #endregion
    ...
    public Segment.Segment GetSegment(int index)
    {
        ...
        if (Count <= 0) return new Segment.Segment();
        var i = index % Count;
        return this[i];
    }
    ...
    {
        var uniqueSegments = new HashSet<Segment.Segment>();
        for (int i = 0; i < Count; i++)
        {
            var seg = this[i];
    ...
    public Segment.Segment GetRandomSegment()
    {
    ...
    /// <returns></returns>
    public int GetCount(Segment.Segment seg) { return this.Count((s) => s.Equals(seg)); }
    ...
    /// <param name="seg"></param>
    /// <returns></returns>
    public int GetCountSimilar(Segment.Segment seg) { return this.Count((s) => s.IsSimilar(seg)); }
    ...
    public bool ContainsSegment(Segment.Segment seg)
    {
    ...
    public bool ContainsSegmentSimilar(Segment.Segment seg)
    {
