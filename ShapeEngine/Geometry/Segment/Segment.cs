...
public readonly struct Segment : IEquatable<Segment>
{
    ...
    public static bool OverlapSegmentCircle(Vector2 segStart, Vector2 segEnd, Vector2 circlePos, float circleRadius) => Circle.Circle.OverlapCircleSegment(circlePos, circleRadius, segStart, segEnd);
    
    public static bool OverlapSegmentTriangle(Vector2 segStart, Vector2 segEnd, Vector2 a, Vector2 b, Vector2 c)
    {
        if (Triangle.Triangle.ContainsTrianglePoint(a, b, c, segStart)) return true;
    ...
    {
        if (Quad.Quad.ContainsQuadPoint(a, b, c, d,  segStart)) return true;
    ...
    public static bool OverlapSegmentPolygon(Vector2 segStart, Vector2 segEnd, List<Vector2> points)
    {
        ...
        for (var i = 0; i < points.Count; i++)
        {
            ...
            if(Polygon.Polygon.ContainsPointCheck(p1, p2, segStart)) oddNodes = !oddNodes;
        }
        return oddNodes;
    }
    ...
    public bool OverlapShape(Line.Line l) => OverlapSegmentLine(Start, End, l.Point, l.Direction);
    public bool OverlapShape(Ray.Ray r) => OverlapSegmentRay(Start, End, r.Point, r.Direction);
    public bool OverlapShape(Circle.Circle c) => OverlapSegmentCircle(Start, End, c.Center, c.Radius);
    ...
    public bool OverlapShape(Polygon.Polygon poly)
    {
        ...
        for (var i = 0; i < poly.Count; i++)
        {
            ...
            if(Polygon.Polygon.ContainsPointCheck(a, b, Start)) oddNodes = !oddNodes;
        }
        return oddNodes;
    }
    ...
    public (CollisionPoint a, CollisionPoint b) IntersectTriangle(Triangle.Triangle triangle) => IntersectSegmentTriangle(Start, End, triangle.A, triangle.B, triangle.C);
    ...
    public CollisionPoints? IntersectPolygon(Polygon.Polygon polygon, int maxCollisionPoints = -1) => IntersectSegmentPolygon(Start, End, polygon, maxCollisionPoints);
    public CollisionPoints? IntersectPolyline(Polyline.Polyline polyline, int maxCollisionPoints = -1) => IntersectSegmentPolyline(Start, End, polyline, maxCollisionPoints);
