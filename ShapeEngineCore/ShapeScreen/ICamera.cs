    ...
    public class CameraAdvanced : ICamera
    {
        ...
        */
        ...
        public void Update(float dt, float curWindowWidth, float curGameWidth)
        {
            ...
            shake.Update(dt);
            Vector2 rawCameraOffset = BaseOffset + shake.Offset + Translation + cameraTweenTotalOffset;
            float rawCameraRotationDeg = BaseRotationDeg + shake.RotDeg + RotationDeg + cameraTweenTotalRotationDeg;
            float rawCameraZoom = ( ( (BaseZoom * ZoomStretchFactor) + shake.Zoom) * ZoomFactor ) / cameraTweenTotalScale;
