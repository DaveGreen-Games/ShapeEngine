...
public class JsonDataExample : ExampleScene
{
    ...
    private class Asteroid
    {
        ...
        public Asteroid(Vector2 pos, Planet target, AsteroidData data)
        {
            ...
            this.target = target;
            color = new ColorRgba(data.Color[0], data.Color[1], data.Color[2], data.Color[3]);
        }
    ...
    private JsonClassSerializer<AsteroidData> serializer;
    ...
    public JsonDataExample()
    {
        ...
        camera = new();
        serializer = new JsonClassSerializer<AsteroidData>();
        
        var saveDirectoryPath = Game.Instance.SaveDirectoryPath;
        var saveFolder = "JsonDataExamples";
    ...
    private void CreateDefaultSavegameXmlData()
    {
        ...
        var json = serializer.Serialize(slowAsteroid);
        externalDataSavePath.SaveText("slowAsteroid.json", json, null, false ,false);

        var helpText = string.Empty;
        helpText += "You can create your custom asteroid data files here.\n";
        helpText += "ShapeEngine JsonDataExample will load and use them.\n";
    ...
    private List<AsteroidData> LoadXmlData()
    {
        ...
        {
         var externalData = externalDataSavePath.LoadDirectory("*.json");
    ...
    private void DrawDescription(Rect rect)
    {
        ...
        textFont.DrawTextWrapNone("Asteroids are randomly spawned based on json data.", split[0], new(0.5f));
        textFont.DrawTextWrapNone($"You can supply your own json data in the examples savegame folder.", split[1], new(0.5f));
